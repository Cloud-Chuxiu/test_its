...
typedef struct 
{
    ...
    int FT_ID;
    uint8_t read_flag;
    int position;    //舵机相对零点转过的角度 0~4095 为1圈
    int16_t speed;       //舵机速度
    int load;        //舵机负载
    int voltage;     //舵机电压
    int temper;      //舵机温度
    ...
    float last_time_ms;
    uint8_t Rotate_OK; //表示是否可以旋转，值为1表示可以进行后续动作
    uint8_t Step;
    uint8_t LastStep;
    uint16_t Flag;
}FT_STS_t;
...
extern int WritePosEx(FT_STS_t *FT_STS, int16_t Position, uint16_t Speed, uint8_t ACC);//普通写位置指令
