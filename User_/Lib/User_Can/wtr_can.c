...
HAL_StatusTypeDef CANFilterInit(CAN_HandleTypeDef *hcan)
{
    ...
    if (hcan == &hcan1) {
        ...
        sFilterConfig.FilterFIFOAssignment = CAN_RX_FIFO0; // 原先是FIFO0
        ...
        if (HAL_CAN_ActivateNotification(hcan, CAN_IT_RX_FIFO0_MSG_PENDING) != HAL_OK) // FIFO0
        {
            Error_Handler();
        }
        ...
        sFilterConfig.FilterMaskIdHigh     = 0x0000;
        sFilterConfig.FilterMaskIdLow      = 0x0000;
        sFilterConfig.FilterFIFOAssignment = CAN_RX_FIFO1; // 原先是FIFO0
        sFilterConfig.FilterActivation     = ENABLE;
        sFilterConfig.SlaveStartFilterBank = 14;
        ...
        if (HAL_CAN_ActivateNotification(hcan, CAN_IT_RX_FIFO1_MSG_PENDING) != HAL_OK) // FIFO0
        {
            Error_Handler();
        }
